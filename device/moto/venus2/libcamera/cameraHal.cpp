        ...
        if (retVal == NO_ERROR) {
            ...
            if (retVal == NO_ERROR) {
                ...
                if (!err) {
                    ...
                    switch (lcdev->previewFormats) {
                        case OVERLAY_FORMAT_YUV422I:
                            Yuv422iToYuv420sp((char*)vaddr, frame, lcdev->previewWidth, lcdev->previewHeight);
                            break;
                        case OVERLAY_FORMAT_RGBA8888:
                            rgba8888ToYuv420sp((char*)vaddr, frame, lcdev->previewWidth, lcdev->previewHeight);
                            break;
...
int camera_set_preview_window(struct camera_device * device, struct preview_stream_ops *window) {
  ...
  const char *str_preview_format = params.getPreviewFormats();
